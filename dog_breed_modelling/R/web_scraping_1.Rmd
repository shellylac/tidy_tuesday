...
author: "Adapted from - Kristen A, kkakey - https://github.com/kkakey/dog_traits_AKC/"
date: "06/2022"
...
Get rankings of most popular dog breeds in [2021](https://www.akc.org/expert-advice/dog-breeds/the-most-popular-dog-breeds-of-2020/)
```{r}
page <- read_html("https://www.akc.org/expert-advice/dog-breeds/most-popular-dog-breeds-of-2021/")
...
breed_rank_df <- breed_rank %>%
  as.data.frame() %>%
  select(1,2) %>% 
  rename("BREED" = X1, "2021" = X2) %>% 
  filter(str_detect(BREED, "^BREED.*", T))
...
breed_rank <- cbind(Rbreed_rank, links)
...
for (num in seq_along(links)) {
  ...
  if (num==1) {
    ...
    rD <- RSelenium::rsDriver(browser = "firefox", port = 4567L, verbose = T)
    remDr <- rD[["client"]]
  }
  remDr$navigate(url)
  Sys.sleep(10)
